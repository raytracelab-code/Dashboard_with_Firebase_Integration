...
def camera_stream():
    ...
    # DEMO MODE → single JPEG
    if mode == "demo":
        jpg = _demo_camera_frame("DEMO CAMERA (NO LIVE FEED)")
        return Response(jpg, mimetype="image/jpeg")

    # IP CAMERA MODE → LIVE MJPEG STREAM
    try:
        auth = (CAMERA_USERNAME, CAMERA_PASSWORD) if CAMERA_USERNAME else None
        resp = requests.get(
            IP_CAMERA_URL,
        ...
        jpg = _demo_camera_frame("CAMERA ERROR - FALLBACK")
        return Response(jpg, mimetype="image/jpeg")
...
def capture_image():
    """
    Capture:
    - Try real camera snapshot (best-effort)
    - If fails, generate a demo frame so capture always works
    Stored per report type: vjr_images / vdr_images
    """
    data = request.json or {}
    report_type = data.get("report_type", "vdr")

    # ---- NEW: encoding + firebase options from frontend ----
    upload_firebase = bool(data.get("upload_firebase", False))
    ...
    with image_lock:
        ...
        # ---- Firebase DB-only numeric image upload (NO STORAGE) ----
    firebase_uploaded = False

    if upload_firebase and FIREBASE_ENABLED:
        try:
            # 🔥 THIS WAS MISSING
            init_firebase()
            ...
            db.reference(
                f"vessels/{VESSEL_ID}/numeric_captures"
            ).push(payload)
            ...
            import traceback
            print("🔥 Firebase DB numeric upload error:")
            traceback.print_exc()
...
def get_navigation_data():
    with nav_lock:
        return jsonify({"current": nav_current, "history": nav_history[:200]})
...
<body>
  ...
  <div class="tabs">
    <button class="tab-btn active" onclick="openTab('nav')">Navigation</button>
  ...
  <div class="content-wrapper">
    ...
    <div class="tab-content active" id="tab-nav">
      <div class="grid2">
        <div class="panel">
          <div class="panel-header">Navigation Feed <span class="hint">ST6100-like</span></div>
          <div class="panel-content">
            <div class="raw" id="rawNav">Waiting for NAV data...</div>
            ...
            <div class="data-grid">
              ...
              <div class="card"><div class="label">Speed (kn)</div><div class="value" id="spd">--</div></div>
              <div class="card"><div class="label">COG (deg)</div><div class="value" id="cog">--</div></div>
              <div class="card"><div class="label">Heading</div><div class="value" id="hdg">--</div></div>
              <div class="card"><div class="label">Voltage</div><div class="value red" id="vol">--</div></div>
            </div>

            <div class="row2">
              <button class="btn secondary" onclick="downloadNavCSV()">Download NAV CSV</button>
              <button class="btn secondary" onclick="notify('NAV Feed is live and auto-logged.', 'ok')">Status</button>
            </div>
          </div>
        </div>

        <div class="panel">
          <div class="panel-header">Live Map <span class="hint">Track + marker</span></div>
          <div class="panel-content">
            <div class="mapbox" id="mapNav"></div>
            <div style="height:10px"></div>
            <div class="note">
              Track line is drawn from recent navigation history. This is simulated data for demo-safe operation.
            </div>
          </div>
        </div>
      </div>
    ...
    <div class="tab-content" id="tab-weather">
      <div class="grid2">
        <div class="panel">
          ...
          <div class="panel-content">
            <div class="data-grid">
              <div class="card"><div class="label">Wind Speed</div><div class="value" id="wspd">--</div></div>
              <div class="card"><div class="label">Wind Dir</div><div class="value" id="wdir">--</div></div>
              ...
              <div class="card"><div class="label">Updated</div><div class="value red" id="wts">--</div></div>
            </div>
            <button class="btn secondary" onclick="notify('Weather feed is live (simulated).', 'ok')">Status</button>
          </div>
        </div>
        ...
        <div class="panel">
          ...
          <div class="panel-content">
            ...
            <div style="height:10px"></div>
            <div class="note">
              Weather map is linked to vessel position for contextual awareness.
            </div>
          </div>
        </div>
      </div>
...
<div class="row2" style="margin-top:12px;">
  ...
  <script>
    ...
    function openTab(tab){
      ...
      // map fix for Leaflet
      setTimeout(()=>{ try{ mapNav && mapNav.invalidateSize(); }catch(e){}; try{ mapWeather && mapWeather.invalidateSize(); }catch(e){}; }, 200);
    }

    // Map - NAV
    let mapNav, navMarker, navLine;
    let navLatLngs = [];
    ...
    navLine = L.polyline([], { weight: 3 }).addTo(mapNav);
...
function captureFromPTZ(reportType) {
  const fbUpload = document.getElementById("fbUpload")?.checked || false;
  ...
  try {
    ...
    async function loadCapturedImages(){
      try{
        ...
        vjr.forEach((img, idx)=>{
          ...
          d.innerHTML = `
            <img src="data:image/jpeg;base64,${img.data}" />
            <button class="xbtn" title="Delete" onclick="deleteCapture('vjr', '${img.id || ''}', ${idx})">×</button>
            <div class="cap">${img.timestamp || ""}</div>
          `;
          vjrG.appendChild(d);
        });
        ...
        vdr.forEach((img, idx)=>{
          ...
          d.innerHTML = `
            <img src="data:image/jpeg;base64,${img.data}" />
            <button class="xbtn" title="Delete" onclick="deleteCapture('vdr', '${img.id || ''}', ${idx})">×</button>
            <div class="cap">${img.timestamp || ""}</div>
          `;
          vdrG.appendChild(d);
        });
    ...
    async function fetchNavigationData(){
      try{
        ...
        document.getElementById("rawNav").textContent = cur.raw_string || "";
        document.getElementById("lat").textContent = cur.latitude ?? "--";
        document.getElementById("lon").textContent = cur.longitude ?? "--";
        document.getElementById("spd").textContent = cur.speed ?? "--";
        document.getElementById("cog").textContent = cur.cog ?? "--";
        document.getElementById("hdg").textContent = cur.heading ?? "--";
        document.getElementById("vol").textContent = cur.voltage ?? "--";

        // map update
        const lat = cur.latitude, lon = cur.longitude;
        if (lat && lon){
        ...
        const pts = hist.slice(0, 60).reverse().map(x => [x.latitude, x.longitude]).filter(p => p[0] && p[1]);
    ...
    async function fetchWeatherData(){
      try{
        ...
        const j = await res.json().catch(()=>({}));
        document.getElementById("wspd").textContent = j.wind_speed ?? "--";
        document.getElementById("wdir").textContent = j.wind_direction ?? "--";
        document.getElementById("temp").textContent = j.temperature ?? "--";
        document.getElementById("hum").textContent = j.humidity ?? "--";
        document.getElementById("pres").textContent = j.pressure ?? "--";
        document.getElementById("wts").textContent = j.timestamp ?? "--";
      }catch(e){}
    }
